...
1. **Purchase-side Nota de Crédito/Débito posts as a new purchase, not a reversal.** `compras_service.rs` lets a `compra` be created with `tipo_documento = NOTA_CREDITO`, but the inventory movement is unconditionally `"ENTRADA"` and `contabilidad_service.rs::sincronizar`'s COMPRAS loop doesn't branch on `tipo_documento` at all — a supplier credit note debits Inventario and credits Caja/Cuentas por Pagar exactly like a real purchase. Contrast with the sales side, which correctly reverses (see correction note below). This is a live bug corrupting inventory valuation and payables/cash today, not a missing feature.
2. **No equity (Patrimonio) accounts exist in the chart of accounts.** The `cuentas_contables` seed list (migrate.rs) has zero PATRIMONIO rows — no Capital Social, no Utilidades Retenidas. A balance sheet built from this ledger can never balance (Activo ≠ Pasivo + Patrimonio), and there's no way to book owner capital contributions/draws or a period-end close.
3. **Verify whether `send_to_dgii()` is still mocked in the live sale path.** `ecf_service.rs`/`ecfl_service.rs` contain a fabricated-TrackID mock (`format!("DGII-TRACK-{}", uuid::Uuid::new_v4())`). Docs 10/11 describe a separate `dgii_client.rs` with real auth/send/poll logic. **Confirm which file is actually wired into `main.rs`'s live sale flow** — if the mock path is reachable in production, sales could be marked "Aceptado" without DGII ever validating them. This is the single highest-priority thing to verify before any real customer goes live.
4. **Marketing overclaims exceed what's built.** README.md and the landing page (`apps/web/app/(customer)/page.tsx`) advertise Format 608 and 609 report generation and "listo para certificación PSFE DGII" — none of this exists in code. PSFE certification has hard prerequisites (3 already-certified clients, live DGII send/poll, a complete XML builder) this repo doesn't meet yet. This is a legal/reputational risk if shown to a real customer or auditor, not a docs nitpick.
...
- **PSFE certification claim ("listo para certificación PSFE DGII") is overstated** — see cross-cutting item 4.
...
**Gaps:**
- **Purchase-side Nota de Crédito/Débito bug** — see cross-cutting item 1. Real, live, corrupts numbers today.
- **No equity accounts** — see cross-cutting item 2.
...
1. Fix purchase-side Nota de Crédito/Débito ledger + inventory treatment (accounting, critical, live bug).
2. Add Patrimonio/equity accounts to the chart of accounts (accounting, critical, structural).
3. Confirm whether the mocked `send_to_dgii()` path is reachable from the live sale flow; wire in the real client if not already done (DGII compliance, critical, functional risk).
4. Correct or remove the 608/609/PSFE-readiness marketing claims until they're actually true (DGII compliance, critical, legal/reputational risk).
