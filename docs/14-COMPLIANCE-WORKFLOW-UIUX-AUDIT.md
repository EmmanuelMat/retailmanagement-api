...
1. ~~**Purchase-side Nota de Crédito/Débito posts as a new purchase, not a reversal.**~~ **FIXED in Phase A (branch `fix/phase-a-critical-fixes`).** A purchase `NOTA_CREDITO` now decreases stock (SALIDA movement), logs a caja INGRESO for non-FIADO purchases, and posts a reversed ledger entry (credits 1200/1150, debits 1100 or 2110); `NOTA_DEBITO` is deliberately unchanged. A related fix found during review: the IT-1 declaration's creditable-ITBIS total (`report_service.rs`, `itbis_neto_del_rango`) now nets purchase credit notes out (it previously added them), so IT-1 agrees with the ledger; the 606 is unchanged because it correctly lists a credit note as its own row with `ncf_modificado`. **Known limitation:** the fix assumes a purchase `NOTA_CREDITO` means goods were physically returned to the supplier; a supplier credit note that is only a price/discount adjustment with no goods movement is not distinguished (there is no discriminator on the request and quantity must be > 0, so such a note would decrement stock, or be rejected by the stock guard if the goods were already sold) — a discriminator for this is a Phase B follow-up. **Caveat: rows already synced before this fix are NOT corrected** — any purchase `NOTA_CREDITO` that was previously posted to the ledger with the old wrong direction and whose stock was inflated still needs a separate data remediation; pre-fix credit notes also logged a wrong-direction caja movement (an EGRESO instead of an INGRESO for non-FIADO purchases) and went through the weighted-average `costo` recalculation as if they were purchases, so the product `costo` was polluted too. No backfill was done. To size the problem, run `SELECT id, tenant_id, created_at, total FROM compras WHERE tipo_documento = 'NOTA_CREDITO';` — no UI in `apps/web` can create a purchase `NOTA_CREDITO`, so the affected rows are likely few or none, but this is unverified against production.
...
1. ~~Fix purchase-side Nota de Crédito/Débito ledger + inventory treatment~~ — **FIXED in Phase A** (purchase `NOTA_CREDITO` only; `NOTA_DEBITO` unchanged; rows synced before the fix are not remediated). Known limitation: this assumes a purchase `NOTA_CREDITO` means goods were physically returned; a price/discount-only supplier credit note is not distinguished (Phase B follow-up: add a discriminator). See cross-cutting finding #1 above.
