...
export async function POST(req: NextRequest, { params }: { params: Promise<{ rnc: string; id: string }> }) {
  const { rnc, id } = await params;
  const res = await fetch(`${CORE_HTTP}/v1/staff/tenants/${rnc}/usuarios/${id}/resetear-password`, {
    method: "POST",
