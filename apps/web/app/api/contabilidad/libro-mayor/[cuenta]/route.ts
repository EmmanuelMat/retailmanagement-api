...
export async function GET(req: NextRequest, { params }: { params: Promise<{ cuenta: string }> }) {
  const { cuenta } = await params;
  const qs = req.nextUrl.search;
  const res = await fetch(`${CORE_HTTP}/v1/contabilidad/libro-mayor/${encodeURIComponent(cuenta)}${qs}`, {
