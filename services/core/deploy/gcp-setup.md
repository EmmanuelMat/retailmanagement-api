...
Replace `PROJECT_ID` and `REGION` (e.g. `us-central1`) throughout with your
actual values.
...
```bash
gcloud storage buckets create gs://PROJECT_ID-fiscal-core-uploads \
  --location=REGION \
...
gcloud projects add-iam-policy-binding PROJECT_ID \
  --member="serviceAccount:fiscal-core-runtime@PROJECT_ID.iam.gserviceaccount.com" \
  --role="roles/secretmanager.secretAccessor"

gcloud storage buckets add-iam-policy-binding gs://PROJECT_ID-fiscal-core-uploads \
  --member="serviceAccount:fiscal-core-runtime@PROJECT_ID.iam.gserviceaccount.com" \
...
```bash
# Find your project's numeric ID (needed for the principalSet resource name below)
...
gcloud iam service-accounts add-iam-policy-binding \
  ...
  --member="principalSet://iam.googleapis.com/projects/PROJECT_NUMBER/locations/global/workloadIdentityPools/github/attribute.repository/EmmanuelMat/retailmanagement-api"
```
...
| `GCP_SERVICE_ACCOUNT_EMAIL` | `gh-deployer@PROJECT_ID.iam.gserviceaccount.com` |
| `GCP_RUNTIME_SERVICE_ACCOUNT_EMAIL` | `fiscal-core-runtime@PROJECT_ID.iam.gserviceaccount.com` |
| `ARTIFACT_REGISTRY_REPO` | `fiscal-core` |
| `CLOUD_RUN_SERVICE` | `fiscal-core-backend` |
| `UPLOADS_BUCKET` | `PROJECT_ID-fiscal-core-uploads` |
